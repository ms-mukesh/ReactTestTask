...
import {
  addData,
  addDataOnFirestore, setLoaderStatus,
  setUserListData,
...
const AddData = props => {
  ...
  const addDataOnFirestore = () => {
    if (data.empName.length === 0 || data.empName === '') {
      alert('please enter employe name');
      return;
    } else if (data.designation.length === 0 || data.designation === '') {
    ...
    createUserEntryOnFireStore(data).then(isAdded => {
